...
// Contact & Action Center: closing CTA + network/social links grid.
'use client';
import CTA from '@/components/CTA';
import { useInView } from '@/hooks/useInView';

const CTA_COPY = {
  en: { title: "Your next big idea starts here. Let's make it real!", subtitle: "Let's talk about your project.", button: 'BOOK DISCOVERY CALL', heading: 'Connect / Network' },
  pt: { title: 'Sua próxima grande ideia começa aqui. Vamos torná-la real!', subtitle: 'Vamos conversar sobre seu projeto. A primeira conversa é grátis.', button: 'AGENDAR CONVERSA', heading: 'Conectar / Rede' },
};
...
export default function ContactActionCenter({ locale, className = '' }: { locale: string; className?: string }) {
  ...
  const t = CTA_COPY[locale as keyof typeof CTA_COPY] ?? CTA_COPY.en;

  return (
    <section className={`relative py-20 ${className}`}>
      <div ref={ref} className={`max-w-4xl mx-auto px-6 ${inView ? 'animate-scroll-fade-in-up' : ''}`}>
        <CTA title={t.title} subtitle={t.subtitle} button={t.button} href="/contact" locale={locale} />

        {/* Network Links Grid */}
        <div className="mt-16">
          <h3 className="text-lg font-semibold mb-4 text-center">{t.heading}</h3>

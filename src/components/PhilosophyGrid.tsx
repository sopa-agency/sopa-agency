...
// Home page section: "how we ship" 3-step process + closing CTA card.
'use client';
    ...
    <section className={`relative py-24 flex items-center ${className}`}>
      <div ref={ref} className={`w-full max-w-7xl mx-auto px-6 ${inView ? 'animate-scroll-fade-in-up' : ''}`}>
        ...
        <div className="mb-16 md:w-2/3">
          <h2 className="font-display text-4xl md:text-5xl font-bold mb-4 page-title-anim">
            {data.title}
          </h2>
        ...
        <div className="border border-white/20 bg-black/40 backdrop-blur-sm rounded-2xl p-8 md:p-12 flex flex-col md:flex-row gap-8 items-start md:items-center justify-between page-anim page-anim-d2">
          <div className="md:w-2/3">
            ...
            <p className="text-amber-300 font-medium italic">&ldquo;{data.tagline}&rdquo;</p>
          </div>
          <div className="flex flex-col sm:flex-row md:flex-col gap-4 min-w-max">
            <button
              onClick={() => window.dispatchEvent(new CustomEvent('sopa:navigate', { detail: 'work' }))}
              className="px-6 py-3 rounded-full border border-white/20 hover:bg-white/10 transition-colors text-sm font-semibold cursor-pointer"
            >
              {data.links.work}
            </button>
            <button
              onClick={() => window.dispatchEvent(new CustomEvent('sopa:navigate', { detail: 'contact' }))}
              className="px-6 py-3 rounded-full bg-white text-black hover:bg-amber-300 transition-colors text-sm font-semibold cursor-pointer"
            >
              {data.links.contact}
            </button>
          </div>
